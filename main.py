...
async def voice_start(config: dict | None = None):
    ...
    try:
        cfg = {
            ...
            "index_rate": float(config.get("index_rate", 0.40)),
            ...
            "block_seconds": float(config.get("block_seconds", 0.50)),
            "noise_gate_enabled": bool(config.get("noise_gate_enabled", True)),
            "noise_gate_threshold": float(config.get("noise_gate_threshold", 0.025)),
            "noise_gate_hold_ms": int(config.get("noise_gate_hold_ms", 140)),
            "noise_gate_release_ms": int(config.get("noise_gate_release_ms", 70)),
            "dry_mix": float(config.get("dry_mix", 0.0)),
